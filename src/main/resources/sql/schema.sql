...
(
    ...
    pdf_url                varchar(500) null comment 'PDF文件URL',
    create_by              BIGINT   DEFAULT 0,
    create_time            DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_by              BIGINT   DEFAULT 0,
    update_time            DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- 盖章记录表
CREATE TABLE IF NOT EXISTS seal_stamp_record
(
    id             BIGINT PRIMARY KEY COMMENT '主键ID',
    stamp_no       VARCHAR(64)  NOT NULL UNIQUE COMMENT '盖章记录编号',
    apply_id       BIGINT       NOT NULL COMMENT '申请ID',
    seal_id        BIGINT       NOT NULL COMMENT '印章ID',
    seal_name      VARCHAR(128) NOT NULL COMMENT '印章名称',
    stamper_id     BIGINT       NOT NULL COMMENT '盖章人ID',
    stamper_name   VARCHAR(64)  NOT NULL COMMENT '盖章人姓名',
    pdf_url        VARCHAR(512) NOT NULL COMMENT 'PDF文件URL',
    seal_image_url VARCHAR(512) NOT NULL COMMENT '印章图片URL',
    stamp_time     DATETIME     NOT NULL COMMENT '盖章时间',
    status         TINYINT      NOT NULL COMMENT '盖章状态（1-成功，2-失败）',
    remark         VARCHAR(255) COMMENT '备注',
    create_by      BIGINT   DEFAULT 0,
    create_time    DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_by      BIGINT   DEFAULT 0,
...
(
    id               BIGINT PRIMARY KEY COMMENT '主键ID',
    evidence_no      VARCHAR(64)  NOT NULL UNIQUE COMMENT '存证编号',
    business_type    VARCHAR(32)  NOT NULL COMMENT '业务类型（APPLY-申请，APPROVE-审批，STAMP-盖章）',
    business_id      BIGINT       NOT NULL COMMENT '业务ID（申请ID、审批记录ID等）',
    business_data    TEXT COMMENT '业务数据JSON',
    data_hash        VARCHAR(128) NOT NULL COMMENT '数据哈希值（SHA-256）',
    block_height     BIGINT       NOT NULL COMMENT '区块高度',
    block_hash       VARCHAR(128) NOT NULL COMMENT '区块哈希值',
    transaction_hash VARCHAR(128) NOT NULL COMMENT '交易哈希值',
    previous_hash    VARCHAR(128) NOT NULL COMMENT '上一区块哈希值',
    timestamp        DATETIME     NOT NULL COMMENT '存证时间戳',
